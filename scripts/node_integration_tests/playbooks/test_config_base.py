...
def make_node_config_from_env(role: str, counter: int) -> NodeConfig:
    ...
    node_config.concent = os.environ.get('GOLEM_CONCENT_VARIANT', 'staging')
    node_config.rpc_port = \
        int(os.environ.get(f'GOLEM_{role}_RPC_PORT', 61000 + counter))
    node_config.password = os.environ.get(f'GOLEM_{role}_PASSWORD', 'dupa.8')
    return node_config
...
class DebugTestConfig(TestConfigBase):
    def __init__(self):
        super().__init__()
        self.requestor.log_level = 'DEBUG'
        self.provider.log_level = 'DEBUG'
